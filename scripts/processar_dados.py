"""
processar_dados.py — Gera public/data/processed/dados_consolidados.json
Uso: python scripts/processar_dados.py
"""
...
def main():
    if not DCENTROS.exists():
        sys.exit(f"Arquivo nao encontrado: {DCENTROS}")
    ...
    for _, row in dc.iterrows():
        ...
        loja_info[bcps] = {"id":bcps,"nome":str(row["LOJA"]).strip(),"praca":str(row["PRACA"]).strip(),
            "gestor":str(row["GVO"]).strip(),"gcvo":str(row["GCVO"]).strip(),"grvo":str(row["GRVO"]).strip(),
            "regional":str(row["REGIONAL"]).strip(),"cluster":str(row["CLUSTER"]).strip(),"categoria":str(row["CATEGORIA"]).strip()}
    ...
    for f in files:
        ...
        if not m: continue
        dd, mm, yyyy = m.groups(); data_iso = f"{yyyy}-{mm}-{dd}"; all_dates.append(data_iso)
    ...
    lojas_list = [loja_info.get(lid,{"id":lid,"nome":lid,"praca":"N/D","gestor":"N/D","gcvo":"N/D","grvo":"N/D","regional":"N/D","cluster":"N/D","categoria":"N/D"}) for lid in sorted(ids_presentes)]

    payload = {"indicador":"ID do Cliente","descricao":"% Atendimentos com CPF identificados (com e sem compra) / Total Boletos Loja",
        "unidade":"%","meta":META,"periodo":{"inicio":all_dates[0],"fim":all_dates[-1]},
        "atualizadoEm":datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),
        "pracas":sorted(set(l["praca"] for l in lojas_list)),"gestores":sorted(set(l["gestor"] for l in lojas_list)),
        "consultores":sorted(set(r["consultor"] for r in registros_cons)),
        "lojas":lojas_list,"registros":registros_pdv,"registrosConsultor":registros_cons}
    ...
    print(f"OK: {OUT}")
    print(f"  Periodo: {all_dates[0]} -> {all_dates[-1]} | Meta: {META}%")
    print(f"  Lojas: {len(lojas_list)} | Reg PDV: {len(registros_pdv)} | Reg Consultor: {len(registros_cons)}")
